...
class HaniHunterDownloadProvider(DownloadProvider):
    def __init__(self, name: str, config_reader: AbsConfigReader) -> None:
        super().__init__(name, config_reader)
        self.provider_name = name
        self.provider_type = 'hanihunter_download_provider'
    ...
    def send_torrent_task(self, task: Task) -> TypeError:
        return TypeError("hanihunter doesn't support torrent task")

    def send_magnet_task(self, task: Task) -> TypeError:
        return TypeError("hanihunter doesn't support magnet task")
    ...
    def send_general_task(self, task: Task) -> TypeError:
        ...
        if not (task.url.startswith('https://hanime1.me') or task.url.startswith('https://hanime.tv')):
            return TypeError('hanihunter only support specific resource')
